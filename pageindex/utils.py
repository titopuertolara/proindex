...
# ── Bedrock LLM completion wrappers ─────────────────────────────────────────

def _converse(model: str, messages: list, temperature: float = 0) -> dict:
    """Call Bedrock converse API and return the full response dict."""
...
def llm_completion(model, prompt, chat_history=None, return_finish_reason=False):
    ...
    for i in range(max_retries):
        try:
            response = _converse(model, messages)
...
async def llm_acompletion(model, prompt):
    """Async wrapper around Bedrock converse (runs in thread pool)."""
    max_retries = 10
    ...
    for i in range(max_retries):
        try:
            response = await loop.run_in_executor(
                None, lambda: _converse(model, messages)
            )
...
def llm_converse_with_tools(model, messages, tools, tool_handlers, system=None, max_turns=10):
    """
    Run a Bedrock converse tool-use loop until the model stops calling tools.

    Args:
        model: Bedrock model ID
        messages: list of converse messages
        tools: list of Bedrock toolSpec dicts
        tool_handlers: dict mapping tool name -> callable(input_dict) -> str
        system: optional system prompt (list of text blocks)
        max_turns: safety limit on tool-use rounds
...
# ── Tree / structure utilities (unchanged from original) ────────────────────
...
def get_last_start_page_from_text(text):
    import re
    start_page = -1
    matches_list = list(re.finditer(r"<start_index_(\d+)>", text))
    if matches_list:
...
# ── List / tree conversion utilities ────────────────────────────────────────
